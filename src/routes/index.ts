...
apiRoutes.get('/public/data', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.status(200).json({ publicData: 'hello world', ...response.data, timestamp: new Date().toISOString() });
  } catch (err) {
...
/* WRISTBAND_TOUCHPOINT - Middleware validates JWT access token before executing the route handler. */
